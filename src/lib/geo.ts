...
/**
 * True if point `p` lies inside the closed `polygon` (exterior ring, lon/lat).
 * Ray-casting in raw degrees — exact for a point-in-polygon test (the longitude
 * scale distortion cancels), and the corridor swaths are small enough that the
 * straight-edge approximation between vertices is immaterial.
 */
