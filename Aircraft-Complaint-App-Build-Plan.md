...
- **Scope:** Farnborough (EGLF), Heathrow (EGLL), Gatwick (EGKK). Filter out military, helicopters and very light/hobbyist GA.
...
> Build a mobile-first PWA called "Aircraft Complaint Assistant", deployed to GitHub Pages, with a Cloudflare Worker data proxy. Stack: Vite + React + TypeScript + Tailwind + Leaflet + vite-plugin-pwa; Worker in TypeScript via Wrangler. Work in the phases defined in `Aircraft-Complaint-App-Build-Plan.md` (sections 5–8), one phase per PR, stopping at each phase's Definition of Done for my review.
>
...
- Exact Farnborough **weekday** hours and the precise Condition number (verify against the 20/00871/REVPP decision notice).
- Real **RNAV waypoint coordinates** for the Rwy 24 SIDs / arrival STARs (UK AIP) to replace the seed corridor polylines.
- Whether to add a **paid** data tier later for reliable origin/destination (FR24/FlightAware) — not needed for v1.
- adsb.lol's future **API-key-by-feeding** change — the Worker isolates you from it.
